...
# evaluation dataset
for f in "usim/usim_en.txt" "context_simlex/evaluation_kit_final/data/data_en.tsv.out" "scws/scws.txt" "WiC_dataset/train/train.data.txt.out" "WiC_dataset/dev/dev.data.txt.out" "WiC_dataset/test/test.data.txt.out";
do
    echo "processing ${f}"
...
do
    echo "processing ${f}"
    python3 extract_features.py --input_file ${f}  --max_seq_length 100 --layers ${layers} --model ${model}  --gpu ${cuda} --batch_size 100;
